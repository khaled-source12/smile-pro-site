...
- في 2026-09-26 أصبحت إعدادات GA4 الإنتاجية: Property باسم `Smile Pro — Production`، وWeb Stream باسم `Smile Pro Production Web` ورابط `https://smileproegypt.com`. أصبح اسم حاوية GTM وGoogle tag أيضًا `Smile Pro Production Web`. بقي مشروع `smile-pro-tracking-test.netlify.app` محصورًا في ملفات staging ولا يظهر في إعدادات GA4 أو Cross-domain الإنتاجية.
- دُمج Google Ads tag `GT-WPQDMSF5` في Google tag الجديد `GT-PB6FRJCR` باسم `Smile Pro Production Web`، مع اعتماد إعدادات GA4 الجديدة. أصبح الـTag الواحد يحمل الوجهتين `G-K4989EX8EJ` و`AW-18233409981`. بعد Hard Refresh في GTM اختفى تحذير `No Google tag found in this container` من Tags تحويلات Google Ads، بما فيها `Google Ads - confirmed lead`. لا تضف Google Tag ثانية.
- عطّلت إعدادات Google Tag التلقائية الخاصة بتغيير history والـscroll والروابط الخارجية والنماذج والفيديو والتنزيلات. تبقى User-provided data capability متاحة للـManual Enhanced Conversions، لكن Automatic detection وCSS/JavaScript selectors كلاهما Off. لا توجد Cross-domain conditions. أصبح فلتر GA4 الافتراضي `Internal Traffic` في حالة Inactive بدل Testing، ولذلك لا يختبر البيانات ولا يستبعدها.
